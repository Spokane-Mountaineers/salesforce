...
export default class TicketSubmissionForm extends NavigationMixin(
  LightningElement
) {
  @track subject = "";
  @track description = "";
  ...
  connectedCallback() {
    ...
    if (
      ...
      window.location.pathname.includes("/support/submit-ticket")
    ) {
  ...
  async checkContact() {
    try {
      ...
      if (!contactId) {
        this.error =
          "Unable to find your Contact record. Please contact support.";
      }
    } catch (error) {
