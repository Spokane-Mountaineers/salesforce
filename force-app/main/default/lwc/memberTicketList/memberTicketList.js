...
export default class MemberTicketList extends NavigationMixin(
  LightningElement
) {
  @track tickets = [];
  ...
  wiredTickets(result) {
    ...
    if (data) {
      ...
      this.tickets = data.map((ticket) => ({
        ...
        formattedLastModifiedDate: this.formatDateValue(
        ...
        recordTypeName: ticket.RecordType ? ticket.RecordType.Name : "Standard",
        recordTypeDeveloperName: ticket.RecordType
          ? ticket.RecordType.DeveloperName
          : "Standard"
      }));
  ...
  applyFilters() {
    this.filteredTickets = this.tickets.filter((ticket) => {
      ...
      if (this.selectedRecordTypeFilter !== "All") {
        if (ticket.recordTypeDeveloperName !== this.selectedRecordTypeFilter) {
          return false;
        }
      }
  ...
  async handleRefresh() {
    ...
    try {
      ...
      this.showToast(
        "Error",
        "Unable to refresh tickets. Please try again.",
        "error"
      );
    } finally {
      this.isLoading = false;
  ...
  get recordTypeFilterOptions() {
    ...
    if (this.tickets && this.tickets.length > 0) {
      this.tickets.forEach((ticket) => {
        if (ticket.recordTypeDeveloperName) {
          recordTypes.add(ticket.recordTypeDeveloperName);
        }
      });
    }
    ...
    Array.from(recordTypes).forEach((rt) => {
      const ticket = this.tickets.find((t) => t.recordTypeDeveloperName === rt);
      if (ticket) {
        options.push({
          label: ticket.recordTypeName,
          value: rt
        });
      }
    });
